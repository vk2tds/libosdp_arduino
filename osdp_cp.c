...
#define ASSERT_BUF_LEN(need)                                                   \
	...
	if (ISSET_DEBUGFLAG(pd, CONFIG_OSDP_DATA_TRACE)) { // vk2tds
...
static int cp_send_command(struct osdp_pd *pd)
{
	...
	if (ISSET_DEBUGFLAG(ps, CONFIG_OSDP_PACKET_TRACE)) { // vk2tds
		if (pd->cmd_id != CMD_POLL) {
...
osdp_t *osdp_cp_setup(int num_pd, osdp_pd_info_t *info_list, uint8_t *master_key)
{
	...
	for (i = 0; i < num_pd; i++) {
		...
		if (IS_ENABLED(CONFIG_OSDP_SKIP_MARK_BYTE)) { //vk2tds
